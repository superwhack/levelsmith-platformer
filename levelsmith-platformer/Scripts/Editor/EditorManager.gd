...
## Runs when the node first enters the tree. Used for connecting signals.
func _ready() -> void:
...
func _process(_delta : float) -> void:
	...
	# Set placeable flag based on bounds and tools.
	var hoveredTileId = tileMap.get_cell_source_id(currentMousePosition);
	isPlaceable = !check_out_of_bounds(currentMousePosition);
	if (toolManager.currentTool != Global.Tool.CURSOR && hoveredTileId >= tileCount): isPlaceable = false; 
	if (toolManager.currentTool == Global.Tool.CURSOR && hoveredTileId < tileCount && hoveredTileId >= 0): isPlaceable = false;
