...
## Finds the player node in the play state to obtain a reference
func try_find_player() -> void:
	if !searchForPlayer:
		return

	## Last node in player group is the player in the play state.
...
func process_edge_scrolling(delta: float) -> void:
	...
	var edgeScrollFactor: float = 1.5;

	# Left edge
	if mousePosition.x <= edgeScrollMargin:
		edgeMovement.x -= edgeScrollFactor;

	# Right edge
	elif mousePosition.x >= viewportSize.x - edgeScrollMargin:
		edgeMovement.x += edgeScrollFactor;

	# Top edge
	if mousePosition.y <= edgeScrollMargin:
		edgeMovement.y -= edgeScrollFactor;

	# Bottom edge
	elif mousePosition.y >= viewportSize.y - edgeScrollMargin:
		edgeMovement.y += edgeScrollFactor;
...
## Receives the mouse scrolling inputs to zoom the camera
